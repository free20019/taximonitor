...
public class SSlichengAction implements Action{
	...
	private SSlichengDao l=new SSlichengDao();
	...
	public String findshizai(){
		...
		for (int i = 0; i < 6; i++) {
			double j=0;
			j=l.findmeiyou(i);
			BigDecimal bg = new BigDecimal(j/10);
	...
	public String findshizaiexcle(){
		ShiZaiDao y=new ShiZaiDao();
		try {
			List<LOADONLINE>list=new ArrayList<LOADONLINE>();
			List<List<LOADONLINE>>aa=new ArrayList<List<LOADONLINE>>();
			List<String>list2=new ArrayList<String>();
			...
			String dateStr2 = new Timestamp(date.getTime()).toString();
				...
				WritableSheet ws = wwb.createSheet("单车平均实载里程分析",0);//创建Excel工作表 指定名称和位置 
				Label label ;
            	label = new Label(0,0,"实载里程(时/分)"); 
            	ws.addCell(label);
            	...
            	ws.addCell(label);
                	...
                	for (int i = 0; i < shangnian.size(); i++) {
                    	label = new Label(i+1,9,shangnian.get(i).getActual_load_mileage()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>shangyue=loadonline.getYy().get(1);
                	for (int i = 0; i < shangyue.size(); i++) {
                		
                    	label = new Label(i+1,8,shangyue.get(i).getActual_load_mileage()); 
                    	ws.addCell(label);
					}
                	String[] max=halfMonth.getActualloadmax();
	            	for (int i = 0; i < max.length; i++) {
						label=new Label(i+1,6,max[i]);
						ws.addCell(label);
					}
	            	String[] min=halfMonth.getActualloadmin();
                	...
                	for (int i = 0; i < shangzhou.size(); i++) {
                    	...
                    	label = new Label(i+1,4,shangzhou.get(i).getActual_load_mileage()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>qiantian=loadonline.getYy().get(3);
                	for (int i = 0; i < qiantian.size(); i++) {
                		
                    	label = new Label(i+1,3,qiantian.get(i).getActual_load_mileage()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>zuotian=loadonline.getYy().get(4);
                	for (int i = 0; i < zuotian.size(); i++) {
                		
                    	label = new Label(i+1,2,zuotian.get(i).getActual_load_mileage()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>jintian=loadonline.getYy().get(5);
                	for (int i = 0; i < jintian.size(); i++) {
                		
                    	label = new Label(i+1,1,jintian.get(i).getActual_load_mileage()); 
                    	ws.addCell(label);
					}
	...
	public SSlichengDao getL() {
		return l;
	}
	public void setL(SSlichengDao l) {
		this.l = l;
	}
